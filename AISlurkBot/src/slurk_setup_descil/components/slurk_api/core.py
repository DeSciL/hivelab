...
@asynccontextmanager
async def patch(api_token, uri, json=None):
    headers = {
        "Authorization": f"Bearer {api_token}",
        "Content-Type": "application/json",
        "Accept": "application/json",
    ...
    async with aiohttp.ClientSession() as session:
        async with session.patch(uri, headers=headers, json=json) as resp:
            yield resp


@asynccontextmanager
async def delete(api_token, uri):
    headers = {
        "Authorization": f"Bearer {api_token}",
    }
    async with aiohttp.ClientSession() as session:
...
async def get_user_permissions(uri, api_token, user_id):
    async with get(api_token, uri + f"/slurk/api/users/{user_id}/permissions") as r:
        r.raise_for_status()
        return await r.json()


async def patch_user_permissions(uri, api_token, user_id, new_permissions):
    async with patch(
        api_token, uri + f"/slurk/api/users/{user_id}/permissions", new_permissions
    ) as r:
        result = await r.json()
...
async def redirect_user(
    ...
    await remove_user_from_room(slurk_uri, token, user_id, from_room_id)
    await add_user_to_room(slurk_uri, token, user_id, to_room_id)
    await sio.emit("room_created", {"room": to_room_id, "task": task_id})


async def remove_user_from_room(slurk_uri, token, user_id, room_id):
...
async def delete_user_token(slurk_uri, token):
    async with delete(
        token,
        f"{slurk_uri}/slurk/api/tokens/{token}",
    ) as response:
        if not response.ok:
            response.raise_for_status()
        ...
        "html": [
            {
                ...
                "id": "",
                "layout-content": f"""
                    var url = new URL("{forward_url}");
                    url.searchParams.append('token', token.id);
