...
namespace SlurkExp.Pages.Sessions
{
    [Authorize]
    public class IndexModel : PageModel
    {
        ...
        public List<Session> SlurkSessions { get; set; } = new List<Session>();
        ...
        public async Task<IActionResult> OnGet([FromRoute] string id, [FromQuery] int? p, [FromQuery] int pageSize = 15)
        {
            ...
            Paging.TotalItems = await _slurkContext.Sessions.CountAsync();
            SlurkSessions = await _slurkContext.Sessions.OrderByDescending(x => x.Id).Skip(offset).Take(pageSize).ToListAsync();
