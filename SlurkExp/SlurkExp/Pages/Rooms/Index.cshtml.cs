...
namespace SlurkExp.Pages.Rooms
{
    [Authorize]
    public class IndexModel : PageModel
    {
        ...
        public List<Room> SlurkRooms { get; set; } = new List<Room>();
        ...
        public async Task<IActionResult> OnGet([FromRoute] string id, [FromQuery] int? p, [FromQuery] int pageSize = 15)
        {
            ...
            Paging.TotalItems = await _slurkContext.Rooms.CountAsync();
            SlurkRooms = await _slurkContext.Rooms.OrderByDescending(x => x.Id).Skip(offset).Take(pageSize).ToListAsync();
