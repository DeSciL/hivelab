...
namespace SlurkExp.Pages.Treatments
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet([FromRoute] string id, [FromQuery] int? p, [FromQuery] int pageSize = 15)
        {
            ...
            Paging.TotalItems = await _context.Treatments.CountAsync();
            Treatments = await _context.Treatments.Include(g => g.Groups).OrderBy(x => x.TreatmentId).Skip(offset).Take(pageSize).ToListAsync();
            return Page();
        }
    }
}
