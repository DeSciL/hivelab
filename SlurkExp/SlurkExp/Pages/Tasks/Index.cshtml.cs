...
namespace SlurkExp.Pages.Tasks
{
    [Authorize]
    public class IndexModel : PageModel
    {
        ...
        public List<Data.SlurkDb.Task> SlurkTasks { get; set; } = new List<Data.SlurkDb.Task>();
        ...
        public async Task<IActionResult> OnGet([FromRoute] string id, [FromQuery] int? p, [FromQuery] int pageSize = 15)
        {
            ...
            Paging.TotalItems = await _slurkContext.Tasks.CountAsync();
            SlurkTasks = await _slurkContext.Tasks.OrderByDescending(x => x.Id).Skip(offset).Take(pageSize).ToListAsync();
