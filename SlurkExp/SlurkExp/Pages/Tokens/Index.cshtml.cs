...
namespace SlurkExp.Pages.Tokens
{
    [Authorize]
    public class IndexModel : PageModel
    {
        ...
        public List<Token> SlurkTokens { get; set; } = new List<Token>();
        ...
        public async Task<IActionResult> OnGet([FromRoute] string id, [FromQuery] int? p, [FromQuery] int pageSize = 15)
        {
            ...
            Paging.TotalItems = await _slurkContext.Tokens.CountAsync();
            SlurkTokens = await _slurkContext.Tokens.OrderByDescending(x => x.Id).Skip(offset).Take(pageSize).ToListAsync();
